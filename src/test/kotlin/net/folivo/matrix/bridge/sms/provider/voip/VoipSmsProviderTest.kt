...
@SpringBootTest(
    properties = [
        "matrix.bridge.sms.provider.voip.enabled=true",
        "matrix.bridge.sms.provider.voip.baseUrl=http://localhost:5100",
        "matrix.bridge.sms.provider.voip.username=username",
...
private fun testBody(
    ...
    return {
        ...
        describe(VoipSmsProvider::getAndProcessNewMessages.name) {
            beforeTest {
                mockServerClient
                    .`when`(
                        HttpRequest.request()
            ...
            it("should save last processed message") {
                db.delete<VoipSmsProcessed>().all().awaitFirstOrNull()
                db.select<VoipSmsProcessed>().first().awaitFirstOrNull()
                    ?.shouldBeNull()
                cut.getAndProcessNewMessages()
                db.select<VoipSmsProcessed>().first().awaitFirstOrNull()
        ...
        describe(VoipSmsProvider::sendSms.name) {
            it("should call Voip device to send message") {
                mockServerClient.`when`(
                    HttpRequest.request()
                        .withMethod(HttpMethod.POST.name)
                        .withPath("/messages/out")
            ...
            describe("on error BadRequest") {
                it("should rethrow error") {
                    ...
                    } catch (error: VoipSmsProviderException) {
            ...
            describe("on other error") {
                ...
                it("should save message to send later") {
                    ...
                    val out = db.select<VoipOutSmsMessage>().all().asFlow().toList()
        ...
        describe(VoipSmsProvider::sendOutFailedMessages.name) {
            ...
            describe("there are failed messages") {
                beforeTest {
                    db.insert(VoipOutSmsMessage("+491234511", "some body 1")).awaitFirstOrNull()
                    db.insert(VoipOutSmsMessage("+491234522", "some body 2")).awaitFirstOrNull()
                ...
                it("should send all messages and delete messages") {
                    ...
                    db.select<VoipOutSmsMessage>().all().asFlow().count().shouldBe(0)
                }
                describe("default room is given") {
                    ...
                    it("should not notify when sending failed") {
                        ...
                        shouldThrow<VoipSmsProviderException> {
                            cut.sendOutFailedMessages()
                        }
                        coVerify { matrixClientMock wasNot Called }
                    }
        ...
        afterTest {
            ...
            db.delete<VoipSmsProcessed>().all().awaitFirstOrNull()
            db.delete<VoipOutSmsMessage>().all().awaitFirstOrNull()
        }
    }
}